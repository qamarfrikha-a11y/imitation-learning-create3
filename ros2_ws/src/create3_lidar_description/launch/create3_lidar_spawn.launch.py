...
ARGUMENTS = [
    ...
    DeclareLaunchArgument('spawn_dock', default_value='true', choices=['true', 'false']),
...
def generate_launch_description():
    ...
    create3_nodes_launch_file = PathJoinSubstitution([pkg_create3_common_bringup, 'launch', 'create3_nodes.launch.py'])
    dock_description_launch_file = PathJoinSubstitution([pkg_create3_common_bringup, 'launch', 'dock_description.launch.py'])
    robot_description_launch_file = PathJoinSubstitution([pkg_lidar_description, 'launch', 'robot_description_lidar.launch.py'])
    rviz2_launch_file = PathJoinSubstitution([pkg_create3_common_bringup, 'launch', 'rviz2.launch.py'])
    ...
    spawn_robot_group_action = GroupAction([
        ...
        Node(
            package='gazebo_ros', executable='spawn_entity.py', name='spawn_standard_dock',
            arguments=['-entity', dock_name, '-topic', 'standard_dock_description',
                       '-x', x_dock, '-y', y_dock, '-z', z, '-Y', yaw_dock],
            output='screen', condition=IfCondition(spawn_dock),
        ),
        IncludeLaunchDescription(
        ...
        Node(
            package='gazebo_ros', executable='spawn_entity.py', name='spawn_create3',
    ...
    delayed_spawn = TimerAction(period=8.0, actions=[spawn_robot_group_action])
